        ...
        <tr>
          ...
          <td>
            ...
            <div class="alert alert-warning kda_network_analysis" style="margin: 0 auto; width: 90%;margin-top: 10px;display: none;">
              <i class="icon-warning-sign" style="margin-right: 6px;font-size: 12px;"></i>Currently, for network repositioning in KDA to PharmOmics, we query only meta signatures to save computational load. You may run network repositioning using all drug signatures (meta and dose/time segregated) on your associated genesets from our indepedent <a href="http://mergeomics.research.idre.ucla.edu/runpharmomics.php">PharmOmics pipeline</a> which will require a login. We are working to include more sample tissue-specific networks.
            </div>
  ...
  <div id="modulesection" class="row" <?php if (isset($_POST['modselect']) ? $_POST['modselect'] : null) {
    ...
    <div class="table-responsive">
      ...
      <table id="moduleselecttable" class="table table-striped table-bordered" cellspacing="0" style="width: 100%;text-align: center;">
        <caption>Showing modules with KDs having FDR < 0.05.</caption> <thead>
        <tr>
...
?>
  <script type="text/javascript">
    ...
    $('#kda2pharmdataform').submit(function(e) {
      ...
      $.ajax({
        'url': 'kda2pharmomics_parameters.php',
        'type': 'POST',
        'data': form_data,
        processData: false,
        contentType: false,
...
<script type="text/javascript">
  ...
  $('.selectholder.kda2pharm .selectdropdown span').click(function() {
    ...
    if (select == 1)
      ...
      $("#alert2").eq(0).html(uploadalert).hide().fadeIn(300);
    else
