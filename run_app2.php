...
$sshpass_cmd_in_R="cat(paste0(\"~/bin/sshpass -p '".$env["MERGEOMICS_SERVER_PASSWORD"]."' scp -r \", drugNetworksDir,\" ".$env["MERGEOMICS_SERVER_USERNAME"]."@".$env["MERGEOMICS_SERVER_IP"].":/var/www/mergeomics/html/Data/Pipeline/Resources/shinyapp2_temp/\", folder,\"/\n\"))" . "\n".
                  ...
                  "cat(paste0(\"~/bin/sshpass -p '".$env["MERGEOMICS_SERVER_PASSWORD"]."' scp \", data_dir, sessionID,\"_app2result.txt ".$env["MERGEOMICS_SERVER_USERNAME"]."@".$env["MERGEOMICS_SERVER_IP"].":/var/www/mergeomics/html/Data/Pipeline/Results/shinyapp2/\"))"."\n".
                  ...
                  "cat(paste0(\"~/bin/sshpass -p '".$env["MERGEOMICS_SERVER_PASSWORD"]."' scp \", data_dir, sessionID, \"out.txt ".$env["MERGEOMICS_SERVER_USERNAME"]."@".$env["MERGEOMICS_SERVER_IP"].":/var/www/mergeomics/html/Data/Pipeline/Results/shinyapp2/\"))"."\n".
                  "system(paste0(\"~/bin/sshpass -p '".$env["MERGEOMICS_SERVER_PASSWORD"]."' scp \", data_dir, sessionID, \"out.txt ".$env["MERGEOMICS_SERVER_USERNAME"]."@".$env["MERGEOMICS_SERVER_IP"].":/var/www/mergeomics/html/Data/Pipeline/Results/shinyapp2/\"))"."\n";
$fp = fopen($fpathOut, "w");
fwrite($fp, $data);
fwrite($fp, $analysis);
