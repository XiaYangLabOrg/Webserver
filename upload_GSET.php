<?php
//**This upload page is used to upload a Gene Set File. This page is shared with all 4 pipelines (MDF, MSEA, META, wKDA)
/*
...
*/
 ...
 if (isset($_FILES['GSETuploadedfile']['name'])) //checks if the file that was uploaded exists
 {
    if(isset($_POST['rmchoice']) ? $_POST['rmchoice'] : null) { //checks which pipeline the user is uploading from
        $rmchoice = $_POST['rmchoice'];
                ...
                {
                    $target_path = "./Data/Pipeline/Resources/ssea_temp/";
                }
	...
	$target_path = $target_path . basename($_FILES['GSETuploadedfile']['name']); //target path to save file on server
    $filename =  basename($_FILES['GSETuploadedfile']['name']); //retrieve the uploaded filename
	$fh = fopen($_FILES['GSETuploadedfile']['tmp_name'], 'r'); //open the uploaded file to start the check
    ...
    if ($fh)  //check if the file was opened correctly
    {
            while ( $index++ < 2 ) //run the loop twice
            {
                ...
                $check = "MODULE\tGENE";
                if(!($rmchoice == 1 or $rmchoice == 2 or $rmchoice == 3)){
                    $check = "MODULE\tNODE";
                }
                if ($line !== false && $index == 1) 
                {
                    ...
                    else if (strpos($line, "MODULE")!== false && strpos($line, "NODE")!== false){
                        echo '1'; //Header contains MODULE and NODE
                    }
                    else
                    {
                ...
                {
                    die('No data/empty file: '.basename($_FILES['GSETuploadedfile']['name']));
                }
    ...
    // error opening the file.
        die('Could not open file: '. basename($_FILES['GSETuploadedfile']['name']));
    } 


    if($_FILES['GSETuploadedfile']['tmp_name'])
    {

		if(move_uploaded_file($_FILES['GSETuploadedfile']['tmp_name'], $target_path))
         {
    ...
    if (fclose($fh) === false) {  //close the file
        die('Could not close file: '.basename($_FILES['GSETuploadedfile']['name']));
        }
