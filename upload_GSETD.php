<?php
//**This upload page is used to upload a Gene Set Description File. This page is shared with all 4 pipelines (MDF, MSEA, META, wKDA)
/*
...
*/
 ...
 if (isset($_FILES['GSETDuploadedfile']['name'])) //checks if the file that was uploaded exists
 {
	...
	$target_path = $target_path . basename($_FILES['GSETDuploadedfile']['name']); //target path to save file on server
    $filename =  basename($_FILES['GSETDuploadedfile']['name']); //retrieve the uploaded filename
	$fh = fopen($_FILES['GSETDuploadedfile']['tmp_name'], 'r'); //open the uploaded file to start the check
    ...
    if ($fh)  //check if the file was opened correctly
    {
            while ( $index++ < 2 ) //run the loop twice
            {
                ...
                {
                    die('No data/empty file: '.basename($_FILES['GSETDuploadedfile']['name'])); //Empty file. End script and do not continue. 
                }
    ...
    // error opening the file.
        die('Could not open file: '. basename($_FILES['GSETDuploadedfile']['name']));
    } 


    if($_FILES['GSETDuploadedfile']['tmp_name'])
    {

		if(move_uploaded_file($_FILES['GSETDuploadedfile']['tmp_name'], $target_path))
         {
    ...
    if (fclose($fh) === false) { //close the file
        die('Could not close file: '.basename($_FILES['GSETDuploadedfile']['name']));
        }
