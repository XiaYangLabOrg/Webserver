<?php
/*
This upload page is used to upload a Marker Association File. This page is shared with all 4 pipelines (MDF, MSEA, META, wKDA)
...
*/
 ...
 if (isset($_FILES['MAFuploadedfile']['name'])) //checks if the file that was uploaded exists
 {
	...
	$target_path = $target_path . basename($_FILES['MAFuploadedfile']['name']); //target path to save file on server
    $filename =  basename($_FILES['MAFuploadedfile']['name']);  //retrieve the uploaded filename
	$fh = fopen($_FILES['MAFuploadedfile']['tmp_name'], 'r'); //open the uploaded file to start the check
    ...
    if ($fh) //check if the file was opened correctly
    {
            while ( $index++ < 2 ) //run the loop twice
            {
                ...
                else if($line == true && $index == 2)
                {
                    ...
                    {
                        echo  "0"; //Header is correct, but secondline does not have data ('10')
                        fclose($fh);
                        exit;
                    }
                ...
                {
                    die('No data/empty file: '.basename($_FILES['MAFuploadedfile']['name'])); //Empty file
                }
    ...
    // error opening the file.
        die('Could not open file: '. basename($_FILES['MAFuploadedfile']['name']));
    } 


    if($_FILES['MAFuploadedfile']['tmp_name'])
    {

		if(move_uploaded_file($_FILES['MAFuploadedfile']['tmp_name'], $target_path))
         {
    ...
    if (fclose($fh) === false) { //close the file
        die('Could not close file: '.basename($_FILES['MAFuploadedfile']['name']));
        }
