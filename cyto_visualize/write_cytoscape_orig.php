...
<?php
    $module_list = "";
    $colors="/home/www/abhatta3-webserver/Data/Pipeline/Results/cytoscape/$sessionID.wKDA_cytoscape_module_color_mapping.txt";
...
<?php
    $edge_list = "edges: [ ";
    $edge_weight = "";
    $edges="/home/www/abhatta3-webserver/Data/Pipeline/Results/cytoscape/$sessionID.wKDA_cytoscape_edges.txt";
...
<?php
    ...
    $node_size = "";
    $url_list = "";
    $nodes="/home/www/abhatta3-webserver/Data/Pipeline/Results/cytoscape/$sessionID.wKDA_cytoscape_nodes.txt";
    ...
    for ($i=1;$i<(count($node_explode)-1);$i++)
    {
        ...
        if (trim($color) == "#909090" && $url != "") //never occurs?
        {
            $url = trim($url);
        ...
        else if(trim($color) != "#909090" && $url != "") // occurs
        {
            $url = trim($url);
        ...
        else if(trim($shape) == "diamond" ) // added by Jess, make KDs big
        {
        	$valign = "center";
        	$fontsize="20px";
            $node_list .= "{data: { id: \"$node\", type: \"$shape\", color: \"$color\", fontsize: \"$fontsize\", valign: \"$valign\", size: \"$size\" }}, \n";
        }
        else if(trim($color) == "#909090" && $url == "") //occurs
        {
            $valign = "top";
            $fontsize="12px";
            $node_list .= "{data: { id: \"$node\", type: \"$shape\", color: \"$color\", fontsize: \"$fontsize\", valign: \"$valign\", size: \"10\" }}, \n";
        }
        else if(trim($color) != "#909090" && $url == "") //never occurs? no url is always #909090
        {
