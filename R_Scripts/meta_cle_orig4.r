...
metassea2kda <- function(job, ssea_jobs, symbols=NULL, filter=NULL, rmax=NULL, individual_cutoffs=NULL) {
    ...
    if(!none_significant){
      combined_data <- read.delim(paste0(job$folder,"/ssea/",gsub("_META","",job$label),"_META.combined.results.txt"))
      combined_data[is.na(combined_data)] <- 1
      for(iter in 1:length(individual_cutoffs)){
        if(individual_cutoffs[iter]==1){
          keep_rows <- combined_data[,paste0(names(individual_cutoffs)[iter],".FDR")] <= individual_cutoffs[iter]
        } else {
          keep_rows <- combined_data[,paste0(names(individual_cutoffs)[iter], ".FDR")] < individual_cutoffs[iter]
        }
      ...
      if(nrow(combined_data)<3){ # only ctrl left
        ...
        cat("\nAfter filtering for individual dataset FDR cutoffs,", length(unique(moddata$MODULE)), "modules will be exported to KDA.\n")
...
#                  NMARKER    number of distinct member loci
...
ssea.finish.fdr <- function(job, jobs=NULL) {
    ...
    if(is.null(jobs)){
      ...
      if(nrow(job$modinfo)>2){
        job$modinfo$MODULE <- job$modules
    ...
    if(!is.null(jobs)){
      ...
      combine_res$META.P = res[,paste0("P.", job$label)]
      combine_res$META.FDR = res[,paste0("FDR.", job$label)]
...
ssea.finish.details <- function(job, jobs=NULL) {
  ...
  for(k in res[mask,"MODULE"]) {
    genset <- mod2genes[[k]]
    tmp <- data.frame(MODULE=k, GENE=genset, NMARKER=gensizes[genset],
  ...
  dtl <- dtl[,c("MODULE", "FDR", "GENE", "NMARKER",
                "LOCUS", "VALUE", "DESCR")]
...
ssea.meta <- function(jobs, label, folder) {
    ...
    meta$results$Cochran.Q <- vapply(meta$results$MODULE, function(x){
      ...
      cochran_res <- cochran.Q(x = xval, weights = weightval)
    ...
    meta$results <- meta$results[!is.na(meta$results$P),]
    meta <- ssea.finish.fdr(meta, jobs = jobs)
    meta <- ssea.finish.details(meta, jobs = jobs)
