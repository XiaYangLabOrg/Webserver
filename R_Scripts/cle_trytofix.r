...
depth=1) {
    ...
    noddata$KD_MODULE <- sapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            return(concatenate(drivers$MODNAMES[drivers$NODNAMES==x], mysep = ","))
        } else{
            return("Not a KD")
        }
    })
    noddata$KD_TOP_MODULE <- sapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            # find top module
    ...
    noddata$KD_BORDER_COLOR <- vapply(noddata$NODE, function(x){
        if(x %in% drivers$NODNAMES){
            # find top module
            kdres <- drivers[drivers$NODNAMES==x,]
            col <- kdres$COLOR[which(min(kdres$FDR)==kdres$FDR)]
            if(length(col)>1){
                col <- col[1]
            }
            return(col)
    ...
    if (!is.null(noddata)) tool.save(frame=noddata[, c("NODE", "LABEL", "COLOR", "SIZE", "SHAPE", "SECTOR","URL", "MODULE","KD_MODULE","KD_TOP_MODULE","KD_BORDER_COLOR","LABELSIZE")], file=nodfile)
...
ssea2kda.import <- function(genfile, locfile) {
    ...
    locdata <- tool.read(locfile, c("MARKER", "VALUE"))
...
ssea.prepare.structure <- function(moddata, gendata, nmods, ngens) {
    ...
    if(sum(gensizes == 0) > 0) stop("Incomplete marker data.")
...
ssea.start <- function(plan) {
    ...
    # Import marker values.
    cat("\nImporting marker values...\n")
    locdata <- tool.read(job$locfile, c("MARKER", "VALUE"))
    locdata$VALUE <- as.double(locdata$VALUE)
    rows <- which(0*(locdata$VALUE) == 0)
    locdata <- unique(na.omit(locdata[rows,]))
    locdata_ex <- locdata
    ...
    gendata <- tool.coalesce(items=gendata$MARKER, groups=gendata$GENE, rcutoff=job$maxoverlap)
...
ssea.start.configure <- function(plan) {
    ...
    if (!is.null(plan$folder) & !is.null(plan$label) & plan$marfile == "undefined" & plan$genfile == "undefined" & plan$modfile == "undefined"){
        ...
        if(is.null(plan$maxoverlap)) plan$maxoverlap <- 0.33
        #if(plan$permtype == "marker") plan$maxoverlap <- 1.0 # no effect
        ...
        if(plan$permtype == "marker") plan$permtype = "marker"
