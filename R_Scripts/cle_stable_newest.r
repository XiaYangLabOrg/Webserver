...
#
kda2cytoscape <- function(job, node.list=NULL, modules=NULL, nmodules=NULL, ndrivers=5, depth=1) {

  # Select top scoring modules.
  cat("\nForwarding KDA results to Cytoscape...\n")

  if(is.null(modules) )  # if module names were not provided by user, take module names from kda results
  ...
  # Select top modules, if nmodules is specified.
  ...
  if(!is.null(node.list)){
	...
	for(ii in 1:length(nds)){
	  if (length(which(job$results$NODE == nds[ii])) > 0 ){
	    mdls = job$results$MODULE[which(job$results$NODE == nds[ii])]
	    drivers = rbind(drivers, cbind(mdls, nds[ii]))
	  }
	  else
	    # drivers = rbind(drivers, c("NON.MODULE", nds[ii]))
	    drivers = rbind(drivers, cbind(0, nds[ii]))
	}
    }
  }
  drivers = as.data.frame(drivers); colnames(drivers) = c("MODULE" , "NODE")

  mods <- unique(drivers$MODULE); modnames <- job$modules[mods] 
  modnames[which(mods == 0)] = "NON.MODULE"
  palette <- kda2cytoscape.colormap(length(mods)) #DIKKAT
  ...
  for(i in 1:length(mods)) {
    rows <- which(drivers$MODULE == mods[i])
    tmp <- kda2cytoscape.exec(job, drivers[rows,], mods, palette, depth)
    if (!is.null(tmp)){
      edgdata <- rbind(edgdata, tmp$edat)
      noddata <- rbind(noddata, tmp$vdat)
    }
  ...
  for ( i in 1:length(unq.nodes) ) {
    ...
    if ( any(is.na(noddata[rws, 5])) ) {
      ...
      noddata[rws, "COLOR"] = strsplit(noddata[rws[1], "SECTOR"], "1:")[[1]][length(strsplit(noddata[rws[1], "SECTOR"], "1:")[[1]])]
  ...
  write.table(instr, sep="\t", file=fname, na="", row.names=FALSE, quote=FALSE, col.names=F)
...
kda2cytoscape.exec <- function(job, drivers, modpool, palette, graph.depth=1) {
  ...
  # Create star topology.
  if(!is.null(job$depth) & (graph.depth > 1) )
    if(graph.depth > job$depth) graph.depth = job$depth 
  ...
  # Select affected nodes.
  tmp <- data.frame(unique(c(edgdata$TAIL, edgdata$HEAD)) , stringsAsFactors=F)
  names(tmp)="NODE"
  # Assign node shapes.
  shapes <- rep("Ellipse", length(tmp))
  sizes <- rep(50, length(tmp))
  tmp$SHAPE = shapes;   tmp$SIZE = sizes; 
  ...
  if(!is.null(noddata)){
    ...
    edgdata$TAIL <- job$graph$nodes[edgdata$TAIL]
    edgdata$HEAD <- job$graph$nodes[edgdata$HEAD]
    modtag <- job$modules[drivers[1,"MODULE"]]; if(length(modtag) == 0) modtag = "NON.MODULE"
...
kda2cytoscape.colorize <- function(noddata, moddata, modpool, palette) {
  ...
  if(length(moddata$NODE) > 0){
    ...
    res <- data.frame(NODE=as.integer(st$labels), COLOR=colors, SECTOR=sectors, URL=urls, LABELSIZE=label.size, stringsAsFactors=FALSE)# keep 1st clr in colors, keep all sectors in sectors
    res <- merge(noddata, res, all.x=TRUE)

    # Fill in missing values.
    rows <- which(is.na(res$COLOR))
    res[rows,"COLOR"] <- "#909090"
    res[which(as.integer(res$SIZE) == 100), "LABELSIZE"] <- 18
    return(res)
   }
  else res=NULL
  return (res)
